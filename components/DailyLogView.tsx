    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 capitalize">{formattedDate}</h2>
                    <p className="text-slate-500 text-sm">Registro diario de actividad</p>
                </div>
                ...
                <div className="space-y-6 md:col-span-2">
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium uppercase tracking-wider text-slate-500">Clima & Observaciones</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-6">
                                ...
                                {(["sunny", "cloudy", "rainy", "windy"] as const).map(w => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            weather === w ? "border-emerald-500 bg-emerald-50 text-emerald-700" : "border-slate-100 bg-white text-slate-400 hover:border-slate-200"
                                        )}
                                    >
                            ...
                            <div className="relative">
                                <StickyNote className="absolute top-3 left-3 w-5 h-5 text-slate-400" />
                                <textarea
                                    className="w-full min-h-[150px] p-3 pl-10 rounded-lg border border-slate-200 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none resize-none text-slate-700"
                    ...
                    <Card className="h-full flex flex-col">
                        ...
                        <CardContent className="flex-1 flex flex-col p-4 gap-4">
                                ...
                                <div className="flex gap-2">
                                    <div className="relative flex-1">
                                        ...
                                        <input
                                            type="number"
                                            placeholder="0.00"
                                            className="w-full p-2 pl-6 rounded border border-slate-200 text-sm"
                                            value={expAmount}
                            ...
                            <div className="flex-1 overflow-y-auto space-y-2 max-h-[300px]">
                                ...
                                {expenses.map(exp => (
                                    <div key={exp.id} className="flex justify-between items-center p-2 bg-white border border-slate-100 rounded shadow-sm group">
                                        <div className="overflow-hidden">
                                            <div className="font-medium text-slate-700 text-sm truncate">{exp.description}</div>
                                            <div className="text-[10px] text-slate-400 truncate">{exp.category}</div>
                                        </div>
                            ...
                            {expenses.length > 0 && (
                                <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                                    <span className="text-xs font-bold uppercase text-slate-500">Total Día</span>
